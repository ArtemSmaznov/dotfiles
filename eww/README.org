           ...
           (eventbox :onclick "eww open --toggle status-panel"
                     ...
                     (box :class "widget music"
                          ...
                          (label :class "music-volume"
                                 :text "${replace(round(music-volume, 2), '0.', '')}%"))))
#+end_src
...
(defwidget music-player []
           (box :class "music-player"
                ...
                (eventbox :onclick "~/.local/bin/playerctl.sh toggle"
                          :cursor "pointer"
...
(defwidget music-player-controls []
           (box :class "music-player-controls"
                ...
                (eventbox :cursor "pointer"
                          (button :onclick "~/.local/bin/playerctl.sh prev"
                                  (label :class "icon"
                                         :text "backward-step")))
                ...
                (scale :class "music-progress-bar"
                       ...
                       :vexpand false
                       :min 0
                       :value { music-track-progress / 1000 / 1000 }
                       :max { music-track-duration / 1000 / 1000 }
                       :active {music-state == "playing"}
                       :onchange "~/.local/bin/playerctl.sh goto {}")

                (eventbox :cursor "pointer"
                          (button :onclick "~/.local/bin/playerctl.sh next"
                                  (label :class "icon"
                                         :text "forward-step")))

                (eventbox :cursor "pointer"
                          (button :onclick "~/.local/bin/playerctl.sh random"
                                  (label :class "icon"
                                         :style "opacity: ${ music-flag-shuffle ? 1 : 0.3 };"
                                         :text "shuffle")))

                (eventbox :cursor "pointer"
                          (button :onclick "~/.local/bin/playerctl.sh single"
                                  (label :class "icon"
...
:END:
** update-music-info
#+begin_src shell :tangle scripts/update-music-info.sh
...
** update-on-song-switch
#+begin_src shell :tangle scripts/update-on-song-switch.sh
eww update music-song="$(~/.local/bin/get-music.sh song)"
eww update music-title="$(~/.local/bin/get-music.sh title)"
eww update music-artist="$(~/.local/bin/get-music.sh artist)"
eww update music-album="$(~/.local/bin/get-music.sh album)"
