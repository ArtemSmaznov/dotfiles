...
[ ! "$XDG_CACHE_HOME" ] && export XDG_CONFIG_HOME="$HOME/.cache"
#+end_src
...
autoload -Uz compinit && compinit -d "$XDG_CACHE_HOME"/zsh/zcompdump-"$ZSH_VERSION" # Load autocompletion
...
if has_command ludusavi; then eval "$(ludusavi complete zsh)"; fi
#+end_src
...
# file is generated by SHELLS.org
#+end_src
...
if uwsm check may-start && ($start_hyprland || uwsm select); then
...
#+begin_src shell
alias biosreboot='sudo systemctl reboot --firmware -setup'
#+end_src

** ZFS
#+begin_src shell
alias zls='zfs list -o name,used,available,referenced,mounted,mountpoint'
alias zll='zfs list -o type,name,used,available,referenced,mounted,mountpoint -t all'
alias zlsnap='zfs list -o type,name,used,referenced,creation -t snapshot'
#+end_src

** AI :ai:
Ollama
#+begin_src shell
# alias ollama='dx ollama ollama'
alias or='ollama run'
alias ops='ollama ps'
...
OpenCode
#+begin_src shell
# alias opencode='dx opencode opencode'
#+end_src
