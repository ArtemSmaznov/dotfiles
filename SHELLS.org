...
Update mirrors
#+begin_src shell
function pman-mirrors-update() {
    curl -s "https://archlinux.org/mirrorlist/?country=CA&protocol=https&use_mirror_status=on" | sed -e 's/#Server/Server/' -e '/^## \w*$/d' | rankmirrors - > /tmp/mirrorlist
