...
if hash eza 2>/dev/null; then
    ...
    alias ls='eza -Fh'                # add file type extensions
...
*** Docker
#+begin_src shell
alias dockercls='docker container ls --format "table {{.ID}}\t{{.RunningFor}}\t{{.Status}}\t{{.Names}}\t{{.Networks}}"'
alias dockercll='docker container ls --all --format "table {{.ID}}\t{{.Image}}\t{{.RunningFor}}\t{{.State}}\t{{.Status}}\t{{.Names}}\t{{.Networks}}"'
#+end_src
