...
* Intro
This is a set of my dotfiles for smaller configs. Larger configurations such as WMs, VIM, etc are hosted on their own separate repositories
...
for repo in "${bare_repos[@]}"; do
    ...
    alias gp$repo="g$repo commit -a && g$repo push"
    alias gd$repo="g$repo pull"
done
#+end_src
